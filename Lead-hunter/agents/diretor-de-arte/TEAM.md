...
| **Nobara** | Criadora de Demo — **executa o BRIEF do Nanami** (escreve o `index.html` do ZERO, stack livre por `motion_tier`) → publica | Operador | Discord | `frontend-design`, stack (three/GSAP/Lenis vendorados), `demo-data`/`similar`/`publicar`, QA (verifica-interface, qa-visual, a11y) |
...
Sukuna prioriza um lead quente (sem site / site ruim, consolidado)
   ...
   │                └──► Nobara: executa o BRIEF (escreve index.html do ZERO, stack por motion_tier → QA → publica) → entrega o LINK
...
> **Regra do handoff da demo:** a Nobara **só cria a partir do BRIEF do Nanami** (o
> `demo-publicar` exige BRIEF real e o `visual-gate` barra esqueleto repetido/templateado).
> `demo-render`/`demo` (template) é fallback de emergência — não é a rota padrão.
